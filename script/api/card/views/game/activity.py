...
def  is_ditto_activity_in_date(model):
    ...
    ditto_activity_list =  Activity.objects.prefetch_related('server').filter(type=model.type,sdate__lte=model.sdate,edate__gte=model.sdate).exclude(name=model.name)
...
def activity_save(request):
    ...
    try:
        ...
        Calevents.create_event(event_name='[%s]-%s' %(model.type,model.name) , desc=model.info, operator=request.admin.username, start=model.sdate,
                            end=model.edate)
    except Exception,e:
        err_msg = trace_msg()
        ...
        if activity_model and server_id:
            ...
            if activity_cls:
                ...
                if save_log and getattr(activity_cls,'IS_SAVE_LOG'):
                    ...
                    activity.gmp.save_log(admin_id,action_type,resutl,remark1=remark1,remark2=_r["msg"],force=force_log)
        print _r
    except:
        _r["msg"] = trace_msg()
                    ...
                    for act in activity_obj_list:
                        ...
                        if msg_dict:
                            ...
                            if type == '排行榜':
                                ...
                                st = date_to_ts(datetime.datetime.replace(sct + datetime.timedelta(days=1),hour=0, minute=0, second=0))
                                et = date_to_ts(datetime.datetime.replace(sct + datetime.timedelta(days=4),hour=23, minute=59, second=59))
                            ...
                            if type == '每日单笔充值':
                                ...
                                st = date_to_ts(sct)
                                et = date_to_ts(datetime.datetime.replace(sct + datetime.timedelta(days=6),hour=23, minute=59, second=59))
                                msg_dict.insert(0, 2)
                                msg_dict[-1] = et
                                msg_dict[-2] = st
