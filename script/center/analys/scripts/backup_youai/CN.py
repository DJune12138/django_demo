...
class BaseQuery(object):
    ...
    def create_tmp_table(cls,conn,s_obj,backup=None):
        if cls.create_table:
            sid = str(s_obj.id)
            cur = conn.cursor()
            sql = cls.replace_sid('''select DATEDIFF(NOW(),CREATE_TIME)>0 from INFORMATION_SCHEMA.TABLES where TABLE_NAME='player_{{server_id}}_tmp' AND TABLE_SCHEMA='ss_log{{server_id}}';''',sid)
            cur.execute(sql)
            result = cur.fetchone()
            ...
            if is_create_table:#创建表
                ...
                try:
                    sql = cls.replace_sid('''create table player_{{server_id}}_tmp like player_{{server_id}};insert into player_{{server_id}}_tmp select * from player_{{server_id}} group by player_id;create index player_{{server_id}}_tmp_index on player_{{server_id}}_tmp(player_id);''',sid)
                    cur.execute(sql)
                    print cur.fetchall()
                except Exception,e:
                    pass
...
class CenterOrdersQuery(BaseQuery):
    ...
    sql = '''SELECT IF(c.`agent_name` IS NULL,'youai',c.`agent_name`),
                    ...
                    IF(LOCATE('10',c.channel_key)=1,'IOS','Android') 
                    FROM pay_action p
...
class SingleOrdersQuery(BaseQuery):
    ...
    sql = '''SELECT IF(c.`agent_name` IS NULL,'youai',c.`agent_name`),
        ...
        IF(p.`query_id` IS NULL, '', p.`query_id`), 
        IF(p.`order_id` IS NULL, '', p.`order_id`),
        concat(truncate(IF(p.`pay_amount`,p.`pay_amount`,0),2)) AS `R`,
        IF(pc.currency IS NULL,'',pc.currency),
        IF(p.`pay_gold` IS NULL, 0, p.`pay_gold`) AS `pay_gold`, 
        ...
        IF(c.channel_key like '%android%','Android','ios')
        FROM pay_action p
        LEFT JOIN channel c ON p.channel_id=c.id
        LEFT JOIN pay_channel pc ON p.pay_type = pc.id
        LEFT JOIN player_all pl ON p.pay_user=pl.player_id
...
class  SingleServerGold(BaseQuery):
    ...
    sql = '''SELECT if(p.user_type is NULL,'',p.user_type),
            ...
            g.log_user,
            if(g.log_tag>5000 or g.log_tag=0,0,1),
            g.log_tag,
            CONVERT(f2,SIGNED),
            CONVERT(f3,SIGNED),
            IF(g.log_time ,UNIX_TIMESTAMP(g.log_time),0)
            FROM ss_log{{master_id}}.`log_player_coin` g
            LEFT JOIN 
            ss_log{{master_id}}.`player_{{server_id}}_tmp` p ON p.player_id=g.log_user
...
class SingleServerMembers(BaseQuery):
    ...
    sql = '''SELECT p.user_type,
            ...
            UNIX_TIMESTAMP(p.last_time),
            if(cc.f1 IS NULL,p.last_ip,cc.f1),
            if(cc.f3,cc.f3,p.mobile_key) as imei,
            IFNULL(ll.log_level,0) as level,
            if(g.last_gold IS NULL,0,g.last_gold),
            if(cc.f2 IS NULL,'Android',substring_index(substring_index(cc.f2,'|',4),'|',-1))
            FROM ss_log{{master_id}}.player_{{server_id}} p
            LEFT JOIN 
            (SELECT log_user,SUM(CONVERT(f2-f1,SIGNED)) AS log_result FROM ss_log{{master_id}}.log_player_online
                WHERE log_time>='{{sdate}}' AND  log_time<'{{edate}}' GROUP BY log_user
            ) o ON o.log_user=p.player_id 
            LEFT JOIN (
                SELECT ac.log_user,ac.f1,ac.f2,ac.f3 FROM ss_log{{master_id}}.log_check_user AS ac INNER  JOIN
                    (SELECT log_user,MAX(id) AS mid FROM ss_log{{master_id}}.log_check_user GROUP BY log_user) AS bc
                    on ac.log_user=bc.log_user and ac.id=bc.mid
            ) cc ON cc.log_user=p.player_id
            LEFT JOIN (select log_user,MAX(log_level) as log_level from ss_log{{master_id}}.log_player_level group by log_user) AS ll ON p.player_id=ll.log_user
            LEFT JOIN ss_log{{master_id}}.log_last_gold_{{server_id}}_tmp  g ON g.log_user=p.player_id
            WHERE  p.player_name!='' AND p.create_time BETWEEN '{{sdate}}' AND '{{edate}}';
          '''
    ...
    def create_tmp_table(cls,conn,s_obj,backup=None):
        ...
        create_tmp_last_gold_sql = '''
            CREATE TABLE IF NOT EXISTS `log_last_gold_{{server_id}}_tmp` (
            `log_user` integer,
            `last_gold` varchar(100),
            `last_time` datetime NOT NULL,
            `expend` double(17,0),
            `pay_gold` double(17,0),
	        `give_gold` double(17,0)
            );
            insert into log_last_gold_{{server_id}}_tmp
            SELECT g.log_user,
            g.log_now AS last_gold,
            g.log_time AS last_time,c.expend,c.pay_gold,c.give_gold 
            FROM ss_log{{master_id}}.log_gold AS g INNER  JOIN
            (SELECT log_user,MAX(id) AS mid ,sum(if(ROUND(f4-f3)<0 ,ROUND(f4-f3),0)) expend,sum(if( ROUND(f4-f3)>0 ,ROUND(f4-f3),0)) as pay_gold,sum(if(log_type!=1121,ROUND(f4-f3),0)) give_gold
              FROM ss_log{{master_id}}.log_gold   WHERE log_time<'{{edate}}' GROUP BY log_user) AS c
...
class SingleServerLogin(BaseQuery):
    ...
    sql = '''SELECT l.platform,l.channel,l.user_id,l.player_name,l.player_id,l.login_time,l.logout_time,
            substring_index(l.ip_mac,'|',1) as ip,l.mobile_key
            FROM 
            (
            SELECT 
            ...
            IF(o.f1 IS NULL, 0, o.f1) AS login_time,
            IF(o.f2 IS NULL, 0, o.f2) AS logout_time,
            IFNULL((SELECT CONCAT(f1,'|',f2) FROM ss_log{{master_id}}.log_check_user WHERE log_time<o.log_time AND log_user=o.log_user LIMIT 1),CONCAT('','|','')) AS ip_mac,
            IF(p.mobile_key IS NULL ,'',p.mobile_key) AS mobile_key
            FROM ss_log{{master_id}}.log_player_online AS o
            LEFT JOIN ss_log{{master_id}}.player_{{server_id}} AS p ON p.player_id=o.log_user
            WHERE o.log_time >= '{{sdate}}' AND o.log_time<'{{edate}}'  AND player_name!=''
            ) l GROUP BY l.player_id,l.login_time
          '''
...
class SingleServerOnlines(BaseQuery):
    ...
    monthly = False
    daily = True
    #在服务器逻辑进程开启的状态下, 从开启成功开始, 每隔5分钟会记录一次
    sql = '''SELECT IFNULL(f1,0) f1,DATE_FORMAT(log_time,'%Y-%m-%d %H:%i') AS time
            FROM ss_log{{master_id}}.log_playing
...
class SingleServerItemslog(BaseQuery):
    ...
    monthly = False
    daily = True
    #平台名,渠道名,玩家账户,玩家角色名,角色ID,位置,项目名称,物品名称,之前数量,现在数量,发生时间
    sql = '''SELECT 
            IF(p.user_type IS NULL, '', p.user_type) AS platform, 
            IF(p.channel_id IS NULL, '', p.channel_id)  AS channel, 
            IF(p.link_key IS NULL, '', p.link_key) AS user_id, 
            ...
            FROM ss_log{{master_id}}.log_item_stack AS e LEFT JOIN ss_log{{server_id}}.player_{{server_id}}_tmp AS p ON e.log_user = p.player_id
