# -*- coding: utf-8 -*-
from cache.memcached_util import MemcachedUtil, get_value as cache_get_value, CACHE_TYPE, save_cache_key
from models import LogDefine, FieldDefine, Query
...
def get_center_log(mc_util = None):
    key = 'CENTER_LOG'
    return get_value(key, mc_util, lambda x:LogDefine.objects.filter(status = 1))
...
def get_all_logDefine():
    '''
    获取所有的LogDefine对象
    '''
    key = 'All_logDefine'
    return get_value(key, None, lambda args:LogDefine.objects.using('read').all())
    
#*******统计相关*************
from analys.models import Statistic
