...
class GmServermixin(object):
    ...
    def gm_address(self):
        ...
        return 'http://%s:%s' % (self.game_addr, self.get_log_db_config().get('gm_port', '8001'))
...
class ServerModelMysqlMixin(object):
    ...
    def get_created_index(cls, conn, db_name):
        '''获取已经创建的了的索引
        '''
        sql = '''SELECT index_name FROM information_schema.STATISTICS where TABLE_SCHEMA="%s" AND INDEX_NAME!='PRIMARY';''' % (
            db_name)
        created_index_list = []
        cur = conn.cursor()
        cur.execute(sql)
    ...
    def get_create_gang_table_sqls(self):
        ...
        sqls.extend(
            [pi_sql.replace(Gang.get_table_name(), 'gang_%s' % self.id).replace('ON ', 'ON %s.' % self.db_name) for
             pi_sql in gang_index])
        return sqls
    ...
    def create_base_table(self):
        ...
        for sql in self.get_create_player_table_sqls() + self.get_create_gang_table_sqls() + [
            self.get_drop_base_log_sql(), self.get_create_base_log_sql()]:
            try:
...
class Server(models.Model, BaseModelMixin, GmServermixin, ServerModelMysqlMixin):
    ...
    game_db_addr_port = models.CharField(u'游戏数据库IP及端口', max_length=100, default="")
    game_db_name = models.CharField(u'游戏数据库名称', max_length=100, default="")
    game_db_user = models.CharField(u'游戏数据库连接用户名', max_length=100, default="")
    game_db_password = models.CharField(u'游戏数据库连接密码', max_length=100, default="")
    __conn = None
    ...
    def get_mongo_conn(self):
        ...
        try:
            if db_user and db_password:
                uri = "mongodb://{0}:{1}@{2}:{3}/the_database?authMechanism=MONGODB-CR".format(db_user, db_password,
                                                                                               db_address, db_port)
...
class ServerPlayer(object):
    ...
    def update_info(self, key, value):
        try:
            _update_sql = '''update player_%s set `%s`='%s' where player_id=%d  ''' % (
                self.sid, key, value, self.player_id)
...
class Group(models.Model):
    ...
    cdn_url = models.CharField(u'网关公告地址', max_length=100)
    ...
    game_app_key = models.CharField(u'游戏APPKEY', max_length=50)
