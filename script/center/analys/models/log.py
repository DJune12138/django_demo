...
class Log(models.Model, BaseModelMixin):
    ...
    log_user = models.BigIntegerField('角色ID', default=0, null=True, db_index=True)
    ...
    # f6 = models.CharField(max_length=100, default='', null=True, blank=True)
    f6 = models.TextField(default='', null=True, blank=True)
    f7 = models.DateTimeField(null=True, blank=True)
    # f8 = models.CharField(max_length=100, default='', null=True, blank=True)
    f8 = models.IntegerField(null=True, blank=True)
    log_channel = models.CharField('渠道ID', max_length=100, null=True, blank=True)
...
class DictValue(models.Model, BaseModelMixin):
    '''道具字典的价值'''
    dict_id = models.IntegerField('字典表id', default=None, db_index=True)
    dict_name = models.CharField(max_length=50)
    dict_price = models.TextField('存键值', default=lambda: pickle.dumps({}))

    def __unicode__(self):
        return '%s' % self.dict_name

    @property
    def dict(self):  # 解析的第一步 pickle
        return pickle.loads(str(self.dict_price))
    ...
    def dict(self, obj_value):
        for k, v in obj_value.iteritems():
            obj_value[k] = v.encode('utf-8')
        _data = pickle.dumps(obj_value)
        self.dict_price = _data

    @property
    def json_dict(self):  # 解析的第二步 json
        return json.dumps(self.dict, ensure_ascii=False)
