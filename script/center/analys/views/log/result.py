...
from models.statistic import Statistic,Result
from django.db import connection 
from cache import center_cache
from views.base import get_server_list
from models.center import  Channel, Group
import datetime, time
#from collections import OrderedDict
from views.log.exprot_file import QueryExprot
...
def result_list(request, statistic_id=0, show_type='list'):
    ...
    if (request.admin.get_resource('server').count() == 0 or request.admin.is_manager) and (list_server.__len__() == query_server.__len__()):
        server_condition = False
        
    if statistic_id == 1:#新增激活特殊处理，不筛选服务器
        server_condition = False
    
    if (request.admin.get_resource('channel').count() == 0  or request.admin.is_manager) and (list_channel.__len__() == query_channel.__len__()):
