...
from util import trace_msg, datetime_to_timestamp, constant
...
from settings import TEMPLATE_DIRS, MEDIA_ROOT, STATIC_ROOT, STATIC_PATH


class ThreadP(object):
    '''
    创建线程池类
    '''

    def __init__(self, max_num=40):  # 创建一个最大长度为20的队列
...
def threadp(server, poll, roll_broadcasts=[]):
    '''滚服公告(世界公告)的get请求的线程函数'''

    # 如果是网关，不创建线程
    if server.id == constant.gateway_id:
        return

    gmp = GMProtocol(server.id)
    result = gmp.roll_broadcast_query()
    if result:
        result[1]['serverId'] = server.id
        result[1]['serverName'] = server.name
        roll_broadcasts.extend(result)
    poll.add_thread()


def write_notice_json(new_content, server_id):
    """构建notice.json的内容"""

    # 读取旧的内容
    content = {}
    try:
        path = os.path.abspath(os.path.join(os.path.abspath(os.path.join(STATIC_PATH, 'server')), 'notice.json'))
    ...
    # 在旧的内容基础上编写新的内容
    content[str(server_id)] = new_content
            ...
            if request.method == 'POST':
                ...
                if request.REQUEST.get('req_type', '') == 'roll_broadcast_del':
                    ...
                    path = os.path.abspath(
                        os.path.join(os.path.abspath(os.path.join(STATIC_PATH, 'server')), 'notice.json'))
...
def edit_server_active_cfg(request):
    ...
    for item in def_params:
        ...
        list_infos.append({'name': item[1], 'key': item[0], 'value': server_info.get(item[0], negative),
                           'is_modify': item[2], 'remark': remark,
                           'recover_value': recover_server_info.get(item[0], ''),
                           })

    return render_to_response('game/cfg_server_active_info.html',
                              {"list_infos": list_infos, "group_server_dict": group_server_dict, 'sdate': sdate,
                               'edate': edate, 'item_id': item_id, 'title': title, 'selected_server': selected_server})
...
def save_server_active_cfg(request):
    ...
    for item in range(1):
        ...
        save_item = {"title": title, "server_list": server_list, "sdate": sdate.strftime('%Y-%m-%d %H:%M:%S'),
                     "edate": edate.strftime('%Y-%m-%d %H:%M:%S'), "server_info": setting_param,
                     "recover_server_info": recover_param}
...
def setting_task(server, net_id, trigger_time, recover_time, recover_data, setting_data):
    ...
    setting_param = {"req_type": 606, "server_id": server_id, "net_id": net_id, "config_type": 0,
                     "json_param": {"config_json": setting_data}}
    recover_param = {"req_type": 606, "server_id": server_id, "net_id": net_id, "config_type": 0,
                     "json_param": {"config_json": recover_data}}
