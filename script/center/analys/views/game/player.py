...
from util import str_to_datetime, datetime_to_timestamp, timestamp_to_datetime_str, filter_sql, constant
...
def role_info(request,player_id=0,template = 'game/role_info.html'):
    ...
    try:
        ...
        if player_info:                 ##########默认玩家是存在的，不然就报错，player_info=返回[-1] ,历史遗留问题，我就不改
            ...
            player_info[6] = TRIBE_TITLE[player_info[6]] if player_info[6] else ""
...
def player_shutup(request, player_id=0):
    ...
    if seconds>60*60*24*365:
        return HttpResponse("That's too long")
...
@Route()
def send_mail(request,template="game/send_mail.html"):
    server_id = int(request.REQUEST.get("server_id",'') or 0)
    ...
    # 资源类型
    award_type_dict = DictDefine.objects.get(id=constant.award_type_id).json_dict

    # 下级列表
    key_num_list = eval(award_type_dict).keys()
    subordinate_list = '['
    subordinate_list_dict = eval(DictDefine.objects.get(id=constant.subordinate_list_id).json_dict)
    for i in key_num_list:
        dic = '{'
        for one in subordinate_list_dict:
            if i == one.split('.')[0]:
                dic += '"' + one.split('.')[1] + '": ' + '"' + subordinate_list_dict[one] + '", '
        dic = dic.rstrip(', ') + '}'
        subordinate_list += dic + ', '
    subordinate_list = subordinate_list.rstrip(', ') + ']'
...
def get_channel(request):
    data={'ch':'','channel_earn':''}

    ch = list(request.admin.role.first().resource_ids['channel'])

    cha_list = list(request.admin.role.first().resource_ids.get('channel', ''))  #########渠道
    channel_li = Channel.objects.filter(id__in=cha_list)
    chanel_earn = 0
    for v in channel_li:
        chanel_earn += v.allow_earn
    data['ch'] = ch
    data['channel_earn'] =chanel_earn
...
def conference_send_mail(request,template="game/conferen_send_mail.html"):
    ...
    print "22222222222",request.admin.role.first().resource_ids ,player_ids       #############admin.id就是agent
    if player_ids:
        player_ids = json.loads(player_ids)
    if msg:
        _r = {'code':-1, 'msg':'', 'succ': [], 'fail': [] }
        ...
        try:
            ...
            cur = con.cursor()
            sql = "select channel_id from player_%s where player_id =%s" % (server_id,player_ids[0])  ###分服获取当前渠道
            cur.execute(sql)
            res = cur.fetchall()
    ...
    for v in channel_li:
        chanel_earn+=v.allow_earn
    dj=json.dumps(eval(DictValue.objects.get(dict_id=88).json_dict))             #################
    kj = json.dumps(eval(DictDefine.objects.get(id=88).json_dict))
...
def pay(request):
    ...
    try:
        ...
        channel_id = get_channel_id(player_id)
        print channel_id
        channel_key = ''
        ...
        if player_id and pay_gold and charge_id:
            ...
            the_action.channel_key = channel_key
...
def deduction(request):
    ...
    remove_prop_id = int(request.REQUEST.get("remove_prop_id",'') or 0 )
    remove_prop_num = int(request.REQUEST.get("remove_prop_num",'') or 0 )
    remove_vip_exp = int(request.REQUEST.get("remove_vip_exp",'') or 0 )
    delitem = {}
    delitem["1"] = (remove_prop_id,remove_prop_num)
    if not ly_amount and not by_amount and not add_buff_id and not remove_buff_id and not remove_prop_id and not remove_prop_num and not remove_vip_exp:
        msg = "未填写任何参数！"
        return HttpResponse(msg)
    ...
    gmp = GMProtocol(server_id)
    if ly_amount or by_amount or delitem or remove_vip_exp:
        result = gmp.deduction_jade([player_id,ly_amount,by_amount,remove_vip_exp,delitem])
        if result == 0:
            msg += " 扣除成功  \n"
