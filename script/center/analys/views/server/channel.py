...
@Route()
def agent_edit(request, template='server/agent_edit.html'):
    '''平台编辑
    '''
    err_msg = ''
    ...
    try:
        if agent_id:
            model = Agent.objects.prefetch_related('channel', 'server_group', 'server').get(id=agent_id)
            ...
            model = Agent(id=0, name=default_name, username=default_name, password=password)
...
def channel_list(request):
    if request.POST.get('action', '') == 'change_agent_name':
    ...
    agents = Agent.objects.all().prefetch_related('channel', 'server_group', 'server')
    ...
    return render_to_response('server/channel_list.html', locals())
...
def channel_edit(request, channel_id=0):
    ...
    # agents = Agent.objects.using('read').all()
    groups = Group.objects.using('read').all()
    model = None
    if channel_id == 0:
        default_name = request.GET.get('key', '')
        password = ''.join(random.sample('123456789', 6))
        model = Channel(id=0, channel_key=default_name, name=default_name, username=default_name, password=password)
        model_other = ChannelOther()
    else:
        model = Channel.objects.get(id=channel_id)
        model_other, _ = ChannelOther.objects.get_or_create(cid=model.id)

    return render_to_response('server/manage_channel_edit.html', locals())
    ...
    if model.channel_key != '' and model.name != '':
        if Channel.objects.filter(channel_key=model.channel_key).exclude(id=model.id).exists():
            ...
            if group_id:
                ...
                group.channel.add(model)
                model.game_app_key = group.game_app_key
                model.group_name = group.name
                model.group_key = group.group_key
                model.save()
    else:
        err_msg = '所有数据不能为空!'
...
@Route()
def channel_remove(request, channel_id=0):
...
def channel_view(request, query_id=1, channel_id=0):
    ...
    count_sql = 'select count(distinct result_time) result from result where statistic_id in(%s)%s%s%s' % (
    ','.join(join_results), query_channel, where_server, query_date)
    ...
    if total_record > 0:
        ...
        query_sql = 'select %s from result where statistic_id in(%s)%s%s%s group by result_time order by result_time desc' % (
        select_str, ','.join(join_results), query_channel, where_server, query_date)
    ...
    return render_to_response('channel/result_list.html', parg,
                              context_instance=RequestContext(request, processors=[channel_channelLoginStatus]))
...
def channel_list_allchannel(request, query_id=1, channel_id=0):
    ...
    if total_record > 0:
        ...
        query_sql = 'select %s from result where statistic_id in(%s)%s%s%s group by result_time' % (
        select_str, ','.join(join_results), query_channel, where_server, query_date)
        print 'query_sql:'
        print(query_sql)
        cursor.execute(query_sql)
    ...
    return render_to_response('channel/result_list_allchannel.html', parg,
                              context_instance=RequestContext(request, processors=[channel_channelLoginStatus]))
...
def pay_list(request):
    ...
    return render_to_response('channel/pay_list.html', parg,
                              context_instance=RequestContext(request, processors=[channel_channelLoginStatus]))
...
def channel_query_result(request, query_id=0, ):
    ...
    cursor = connections['read'].cursor()
    count_sql = 'select count(distinct result_time) result from result where statistic_id in(%s)%s%s%s' % (
    ','.join(join_results), query_server_str, query_channel_str, query_date)
    ...
    if total_record > 0:
        ...
        query_sql = 'select %s from result where statistic_id in(%s)%s%s%s group by create_time order by create_time desc limit %d,%d' % (
        select_str, ','.join(join_results), query_server_str, query_channel_str, query_date, spos, page_size)
        print(query_sql)
        cursor.execute(query_sql)
        list_record = cursor.fetchall()
    # cursor.close()
...
def player_list(request, server_id=0):
    ...
    if server_id > 0:
        ...
        conn = MySQLdb.connect(host=the_db_config['host'], user=the_db_config['user'], passwd=the_db_config['password'],
                               port=the_db_config.get('port', 3306), db=the_db_config['db'], charset="utf8")
        ...
        if len(query) > 0:
            ...
            if not export:
                ...
                server_id, ' and '.join(query), (page_num - 1) * page_size, page_size)
            else:
                sql2 = 'select player_id,player_name,link_key,login_num,mobile_key,last_time,create_time,status from player_%d where %s order by id desc' % (
...
def user_list(request):
    ...
    return render_to_response('channel/user_list.html', parg,
                              context_instance=RequestContext(request, processors=[channel_channelLoginStatus]))
...
def player_block(request, server_id=0, player_id=0):
    ...
    if player_id > 0:
        server = Server.objects.using('read').get(id=server_id)
        the_db_config = json.loads(server.log_db_config)
        conn = MySQLdb.connect(host=the_db_config['host'], user=the_db_config['user'],
                               port=the_db_config.get('port', 3306), passwd=the_db_config['password'],
                               db=the_db_config['db'], charset="utf8")
        conn.autocommit(True)
        cursor = conn.cursor()
        try:
            ...
            query_sql = 'update player_%d set status=%d where player_id=%d and channel_id=%d' % (
            server_id, the_status, player_id, channel_id)
            cursor.execute(query_sql)
            cursor.close()
        except Exception, e:
            print('lock user error:', e)
...
def player_query(request):
    ...
    if server_id > 0 and keyword != '':
        try:
            ...
            the_db_config = json.loads(server.log_db_config)
            conn = MySQLdb.connect(host=the_db_config['host'], user=the_db_config['user'],
                                   passwd=the_db_config['password'], port=the_db_config.get('port', 3306),
                                   db=the_db_config['db'], charset="utf8")
            conn.autocommit(True)
            cursor = conn.cursor()
            ...
            keyword = keyword.replace('\'', '\'\'')
            query_sql = {
                0: 'select log_result,log_user,f1,log_time from log_create_role where f1 like \'%s%%\' and log_channel in(0,%d)' % (
                keyword, channel_id),
                1: 'select log_result,log_user,f1,log_time from log_create_role where log_user=\'%s\' and log_channel in(0,%d)' % (
                keyword, channel_id),
                2: 'select log_result,log_user,f1,log_time from log_create_role where log_result=\'%s\' and log_channel in(0,%d)' % (
                keyword, channel_id)}.get(key_type, '')
...
def question_list(request, user_id=0):
    ...
    return render_to_response("channel/question_list.html", parg,
                              context_instance=RequestContext(request, processors=[channel_channelLoginStatus]))
...
def manage_question_list_user(request, user_id=0):
    ...
    if user_id > 0:
        if server_id > 0:
            total_record = Question.objects.filter(post_user__id=user_id, post_user__channel_key=channel.key,
                                                   server_id=server_id).count()
            if total_record > 0:
                data_list = Question.objects.filter(post_user__id=user_id, post_user__channel_key=channel.key,
                                                    server_id=server_id)[
        ...
        if server_id > 0:
            ...
            if total_record > 0:
                data_list = Question.objects.filter(post_user__channel_key=channel.key, server_id=server_id)[
                            (page_num - 1) * page_size:page_num * page_size]

        else:
            total_record = Question.objects.filter(post_user__channel_key=channel.key).count()
            if total_record > 0:
                data_list = Question.objects.filter(post_user__channel_key=channel.key)[
                            (page_num - 1) * page_size:page_num * page_size]
...
def channel_pay_rank_list(request):
    ...
    query_sql = "select a.pay_user,sum(a.pay_amount) total_amount from pay_action a where %s group by pay_user order by total_amount desc %s" % (
