...
var GMeditor = function() {
	...
	var reason_phrase_path = {
		...
		default_value : ["111reason_phrase"]
	};
	...
	this.set_result_define = function(txt_target, type_enum) {
		...
		var control_box = [code_path, content_path];
		var txt = $(txt_target).val();
		if ($.trim(txt) != '') {
			content_json = eval('(' + txt + ')');
		}
		$('#gm_editor').remove();
		$('body').append('<div id="gm_editor" style="display:none"></div>');
		var div = $('#gm_editor');
		...
		if (type_enum == GMTypeEnum.form) { 
			...
			control_box.push({type:"value", name:"模板文件,逗号隔开:", json_key:"templates"});
			...
			control_box.push({type:"key_value", name:"返回代码映射", json_key:"desc_field_map"});
		}
	...
	var save_form = function(txt_target){
		content_json['templates'] = $('#templates').val();
	...
	this.set_params = function(txt_target, type_enum){
		...
		$('body').append('<div id="gm_params_editor" style="display:none"></div>');
