...
class RsyncTarget(object):
    ...
    port = 22
    path = ''
    rsync_all_cmd = '''rsync {opts} -e 'ssh -p{port}' --include-from=rsync_include_file  --exclude-from=rsync_exclude_file --backup --backup-dir="../backup/api_`date +%Y-%m-%d_%H_%m`" ./api/  {ip}:{path}'''
    rsync_only_cmd = '''rsync {opts} -e 'ssh -p{port}' --files-from=rsync_only_files --backup --backup-dir="../backup/api_`date +%Y-%m-%d_%H_%m`" ./api/  {ip}:{path}'''
    rsync_local_cmd = '''rsync -avz --files-from=rsync_only_files --backup --backup-dir="../backup/api_`date +%Y-%m-%d_%H_%m`" ./api/  {path}'''
