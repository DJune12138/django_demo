...
def confirm_jingqioceanandr_get_link_id(request):
    return request.POST.get('payType', '')


def confirm_jingqioceanandr(request, pay_channel):
    result_msg = 'FAILURE'
    if not pay_channel:
    ...
    # except Exception, e:
    #     print 'confirm jingqioceanandr has Error:', e
    #
    # if not app_key:
    ...
    app_k = request.POST.get('appId','')
    sign = request.POST.get('sign','')
    currency = "USD"

    app_key = ""
    try:
        app_key = pay_channel.get_config_value(app_k, '')
    except Exception, e:
        print 'confirm jingqioceanandr has Error:', e
