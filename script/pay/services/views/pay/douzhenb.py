...
def confirm_douzhenb_get_link_id(request):
    return request.POST.get('payType', '')


def confirm_douzhenb(request, pay_channel):
    result_msg = 'FAILURE'
    if not pay_channel:
    ...
    val = "%s%s%s%s%s%s%s%s%s%s%s"%(amount,channel_order_id,channel_product_id,enhanced_sign,game_id,game_user_id,plugin_id,private_data,product_id,server_id,user_id)
