...
class MemcachedUtil(object):
    ...
    def __init__(self, memcache_host=['127.0.0.1:11211'], valid_date = 1200):
        ...
        MemcachedUtil.__lock.acquire()
        if MemcachedUtil.__mc == None:
            MemcachedUtil.__mc = memcache.Client(self.memcache_host, debug=1)
        self.mc = MemcachedUtil.__mc
        MemcachedUtil.__lock.release()
    ...
    def set(self, key, value, cache_time = -1):
        if -1 == cache_time:
            cache_time = self.valid_date
        return self.mc.set(key, value, cache_time)
