...
_public_rsa_key_pphelper = None
#这地方有性能问题,需要修改
def get_key(key_name):
    key_value = _rsa_keys.get(key_name,None)
    if key_value == None:
        module_path = os.path.dirname(__file__)
        key_path = os.path.abspath(os.path.join(module_path, '%s.pem'%key_name))
        key_value = open(key_path, "r").read()
        _rsa_keys[key_name] = key_value
...
#    pub_path = os.path.abspath(os.path.join(module_path, "rsa_public_key.pem"))
    ...
    pub_path_pphelper = os.path.abspath(os.path.join(module_path, "rsa_public_key_pphelper.pem"))
...
#    pubk = open(pub_path, "r").read()
    ...
    pubk_pphelper = open(pub_path_pphelper, "r").read()

    return (prik, pubk_ali,pubk_feiliu,prik_feiliu,pubk_google,pubk_pphelper)

#_private_rsa_key, _public_rsa_key_ali,_public_rsa_key_feiliu,_private_rsa_key_feiliu,_public_rsa_key_google,_public_rsa_key_pphelper = module_init()
...
def check_with_rsa(msg, signature,key):
    '''
...
def encrypt_with_rsa_feiliu(msg):
    key = get_key('rsa_public_key_feiliu')
    return encrypt_with_rsa(msg,key)
    

def decrypt_with_rsa_feiliu(msg):
    key = get_key('rsa_private_key_feiliu')
    return decrypt_with_rsa(msg,key)
    

def decrypt_with_rsa_pphelper(msg):
    key = get_key('rsa_public_key_pphelper')
    return decrypt_with_rsa(msg,key)
    

def check_with_rsa_feiliu(msg, signature):
    key = get_key('rsa_public_key_feiliu')
    return check_with_rsa(msg,signature,key)
    

def decrypt_with_rsa_ali(msg):
    key = get_key('youai_rsa_private_key')
    return decrypt_with_rsa(msg,key)

def sign_with_rsa_ali(msg):
    key = get_key('youai_rsa_private_key')
    return sign_with_rsa(msg,key)

def check_with_rsa_ali(msg, signature):
    key = get_key('youai_rsa_public_key_ali')
    return check_with_rsa(msg,signature,key)
    

def check_with_rsa_google(msg, signature):
    key = get_key('rsa_public_key_google')
    return check_with_rsa(msg,signature,key)
    
def check_with_rsa_huawei(msg, signature):
    key = get_key('rsa_public_key_huawei')
    return check_with_rsa(msg, signature, key)
